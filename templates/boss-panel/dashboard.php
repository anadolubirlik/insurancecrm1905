...
// Temsilci performans verilerini çek
$representatives = $wpdb->get_results("
    SELECT r.id, u.display_name AS rep_name, r.monthly_target,
           COALESCE(SUM(CASE WHEN YEAR(p.created_at) = YEAR(CURRENT_DATE) AND MONTH(p.created_at) = MONTH(CURRENT_DATE) THEN p.premium_amount ELSE 0 END), 0) AS monthly_production,
           COALESCE(SUM(p.premium_amount), 0) AS total_production,
           COALESCE(r.monthly_target - SUM(CASE WHEN YEAR(p.created_at) = YEAR(CURRENT_DATE) AND MONTH(p.created_at) = MONTH(CURRENT_DATE) THEN p.premium_amount ELSE 0 END), r.monthly_target) AS remaining,
           COALESCE((SUM(CASE WHEN YEAR(p.created_at) = YEAR(CURRENT_DATE) AND MONTH(p.created_at) = MONTH(CURRENT_DATE) THEN p.premium_amount ELSE 0 END) / r.monthly_target * 100), 0) AS realization_percentage
    FROM $representatives_table r
    LEFT JOIN $users_table u ON r.user_id = u.ID
    LEFT JOIN $policies_table p ON r.id = p.representative_id AND p.status = 'aktif'
    WHERE r.status = 'active'
    GROUP BY r.id, u.display_name, r.monthly_target
    ORDER BY r.id ASC
");
...
<div class="ab-crm-container">
    ...
    <div class="ab-crm-table-wrapper">
        <div class="ab-crm-table-info">
            <span>Toplam: <?php echo count($representatives); ?> temsilci</span>
        </div>
        ...
        <table class="ab-crm-table">
            ...
            <tbody>
                <?php foreach ($representatives as $rep): ?>
                    <tr>
                        <td><?php echo esc_html($rep->rep_name); ?></td>
                        <td><?php echo number_format($rep->monthly_production, 2, ',', '.'); ?></td>
                        <td><?php echo number_format($rep->total_production, 2, ',', '.'); ?></td>
                        <td><?php echo number_format($rep->monthly_target, 2, ',', '.'); ?></td>
                        <td><?php echo number_format($rep->remaining, 2, ',', '.'); ?></td>
                        <td><?php echo number_format($rep->realization_percentage, 2, ',', '.'); ?>%</td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
